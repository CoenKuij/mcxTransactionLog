    ...
    public class Trade
    {
        ...
        public decimal Amount
        {
            ...
            set
            {
                if (Type == Types.SELL)
                {
                    if ((currency == Currency.LTC || currency == Currency.XPM) && value < 0.02M)
                        throw new ArgumentException("Sell amount must be larger then 0.02");
                    else if (value < 0.1M)
                        throw new ArgumentException("Sell amount must be larger then 0.1");
                    else amount = value;
                }
                else if (Type == Types.BUY && value < 0.001M)
                {
                    throw new ArgumentException("Buy amount must be larger then 0.001");
                }
                else
                {
                    amount = value;
