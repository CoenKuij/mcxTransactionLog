...
namespace mcxNOW
{
    public class Api
    {
        const string BaseUrl = "https://mcxnow.com/";
        ...
        public Orders Orders(Currency currency)
        {
            ...
            //orders2 is only available for pro traders, so try orders2 first when fails use orders
            RestRequest request = Get("orders2", true);

            request.AddParameter("cur", currency.Code);
            try
            {
                o = Execute<Orders>(request);
            }
            catch
            {
                request = Get("orders", true);
                request.AddParameter("cur", currency.Code);
                o = Execute<Orders>(request);
            }
            return o;
        ...
        {
            RestRequest request = Get("userinfo", true);
            UserInfo u = new UserInfo(Execute(request));
            return u;
        }
        ...
        {
            RestRequest request = Get("chat",true);
            ...
            //        RootElement = "doc"
            //    }
            ChatResponse c = Execute<ChatResponse>(request);
        ...
        public List<Fund> Funds()
        {
            ...
            IRestResponse response = Execute(request, null);
        ...
        public void Login(string secretKey, string session)
        {
        ...
        public void Login(string username, string password, string code, bool remember)
        {
            RestRequest request = Post("login.html");
