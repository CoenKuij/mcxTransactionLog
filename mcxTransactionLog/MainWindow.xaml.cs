    ...
    public partial class MainWindow : Window
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                fetchTransactionTimer.Interval = new TimeSpan(0, 1, 0);
                fetchTransactionTimer.Start();
                DispatcherTimer updateAliveTick = new DispatcherTimer();
                updateAliveTick.Tick += new EventHandler(UpdateAliveTick);
                updateAliveTick.Interval = new TimeSpan(0, 0, 2);
                updateAliveTick.Start();
            }
            catch (Exception ex)
            {
                if (ex is MySqlException) MessageBox.Show(ex.Message, "MySQL error");
        ...
        {
            transactionData.FetchTransactionHistory();
        }

        private void UpdateAliveTick(object sender, EventArgs e)
        {
            switch (sbiAliveTick.Content.ToString())
            {
                case "":
                    sbiAliveTick.Content = "-";
                    break;
                case "-":
                    sbiAliveTick.Content = "--";
                    break;
